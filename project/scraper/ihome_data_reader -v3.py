...
class House:
    ...
    def price_predict(self):
        ...
        print(
            "your house price is predicted to be:", f"{int(result)*100000:,}", "toman"
        )


def webscrape_page(pagenumber):
    r = requests.get(
        "https://ihome.ir/sell-residential-apartment/th-tehran?min_price=300000000&order_by=published_at&order_by_type=desc&paginate=30&page={}&property_type=residential-apartment".format(
            pagenumber
        )
    )
...
def add_to_database(titles, locations, prices, specs):
    for item1, item2, item3, item4, item5, item6 in zip(
        titles, locations, prices, specs[0::3], specs[1::3], specs[2::3]
    ):
        sellval_bil = 0
        sellval_mil = 0
        ...
        if len(rows) == 0:
            cursor.execute(
                "INSERT INTO myhouses VALUES (%s, %s, %s, %s, %s, %s)",
                (title, location, price, area, year, bedroom),
            )
...
def specific_price_range():
    ...
    cursor.execute(
        "SELECT * FROM myhouses WHERE price BETWEEN %s AND %s",
        (lower_range / 100000, upper_range / 100000),
    )
    for item in cursor:
        print("title:", item[0])
